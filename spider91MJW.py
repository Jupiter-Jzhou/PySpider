...
def main():
    ...
    obj = comunits.send_requests(url_epi, url_section, need="xpath")
    # 每集地址
    ...
    response = comunits.send_requests(vid, url_store, need="response")
    txt = response.text
    index = txt.find('/')
    txt = txt[index:]
    print(txt)
    ...
    url_last = l + "//" + r + txt
    print(url_last)
    response1 = comunits.send_requests(url_last, url_store, need="response")
    txt1 = response1.text()
    print(txt1)
